...
AVrPlayerCharacter::AVrPlayerCharacter()
{
	...
	xpNeededForArcheryLevel = 80;
	playerDamage = strengthLevel;
	playerMaxHealth = 100;
	playerHealth = 100;
	playerBowDamage = 25;
}
...
{
	//Check if the player is below max weight
	...
	//go through each of the 20 inventory slots
	for (int32 i = 0; i < 20; i++)
	{
		//if an inventory slot = 0 which is empty make it equal this items unique number then break
		if (InventorySlots[i] == 0)
		{
			InventorySlots[i] = 1;
			GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("Test"));
			break;
		} //if an inventory slot already has this item break
...
//Inventory
//Check for the item number of the item in the inventory slot then set that item to 0 lower the weight and set the inventory slot to empty
...
void AVrPlayerCharacter::LevelUpConstitution()
{
	constitutionLevel++;
...
void AVrPlayerCharacter::LevelUpArchery()
{
	archeryLevel++;
	xpNeededForArcheryLevel = xpNeededForArcheryLevel * 2.2;
	xpNeededForArcheryLevel = round(xpNeededForArcheryLevel);

	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::FromInt(xpNeededForArcheryLevel));
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, FString::FromInt(archeryLevel));
}
...
void AVrPlayerCharacter::EquipSword()
{
	...
	else if (equippedSwordType == 0)
	{
		playerDamage = playerDamage;
	}
}

void AVrPlayerCharacter::CheckSwordOnBack()
{
	//check and make sure player has a sword, call this before allowing the player to take a sword from his back
	if (playerBronzeSwords >= 1 || playerIronSwords >= 1 || playerSteelSwords >= 1)
	{
		hasSwordOnBack = true;
	}
	else 
	{
		hasSwordOnBack = false;
	}
}

//Get the equipped sword type and remove one of those swords from players inventory, call this right after taking a sword from back
void AVrPlayerCharacter::TakeSwordFromBack()
{
	if (equippedSwordType == 1)
	{
		playerBronzeSwords--;
		...
		if (playerBronzeSwords <= 0)
		{
			for (int32 i = 0; i < 20; i++)
			{
				if (InventorySlots[i] == 16)
				{
					InventorySlots[i] = 0;
					break;
				}
	...
	else if (equippedSwordType == 2)
	{
		...
		if (playerIronSwords <= 0)
		{
			for (int32 i = 0; i < 20; i++)
			{
				if (InventorySlots[i] == 24)
				{
					InventorySlots[i] = 0;
					break;
				}
	...
	else if (equippedSwordType == 3)
	{
		...
		if (playerSteelSwords <= 0)
		{
			for (int32 i = 0; i < 20; i++)
			{
				if (InventorySlots[i] == 32)
				{
					InventorySlots[i] = 0;
					break;
				}
...
//Get the equipped sword type and add one of those swords to player inventory, call this right after the player puts a sword on back
void AVrPlayerCharacter::PutSwordOnBack()
{
	if (equippedSwordType == 1)
	{
		playerBronzeSwords++;
...
void AVrPlayerCharacter::GainCombatXp()
{
	//Gain all the combat xp from the last enemy killed, combatXpToGain is being set from the enemy script
	attackXp = attackXp + combatXpToGain;
	strengthXp = strengthXp + combatXpToGain;
	defenceXp = defenceXp + combatXpToGain;
	constitutionXp = constitutionXp + combatXpToGain;

	//Check if any combat skills can level up
	if (attackXp >= xpNeededForAttackLevel)
	{
		LevelUpAttack();
	}
	if (strengthXp >= xpNeededForStrengthLevel)
	{
		LevelUpStrength();
	}
	if (defenceXp >= xpNeededForDefenceLevel)
	{
		LevelUpDefence();
	}
	if (constitutionXp >= xpNeededForConstitutionLevel)
	{
		LevelUpConstitution();
	}
	//reset combatXpToGain to 0 so the next enemy gives the right amount
	combatXpToGain = 0;
}

void AVrPlayerCharacter::GainArcheryXp()
{
	archeryXp = archeryXp + archeryXpToGain;

	if (archeryXp >= xpNeededForArcheryLevel)
	{
		LevelUpArchery();
	}
	archeryXpToGain = 0;
...
void AVrPlayerCharacter::CheckArrows()
{
	//Make sure the player has arrows, call this before shooting a bow to ensure the player has an arrow to shoot
	if (playerBronzeArrows >= 1 || playerIronArrows >= 1 || playerSteelArrows >= 1)
	{
		hasArrows = true;
	}
	else
...
//Currently only bronze arrows can be shot
//Must update function once player can use other types of arrows
void AVrPlayerCharacter::ShotArrow()
{
	playerBronzeArrows--;
	playerWeight--;

	if (playerBronzeArrows <= 0)
	{
		for (int32 i = 0; i < 20; i++)
		{
			if (InventorySlots[i] == 23)
			{
				InventorySlots[i] = 0;
				break;
			}
